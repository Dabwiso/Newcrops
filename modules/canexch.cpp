 ...
 *                   to prescribed root distribution
 */
double irrigated_water_uptake(Patch& patch, Pft& pft, const Day& day, bool inundated) {
    Patchpft& ppft = patch.pft[pft.id];
    ...
    for (int ly = 0; ly < NSOILLAYER; ly++) {
        ...
        // SSR
        if (!negligible(patch.soil.soiltype.awc[ly] - patch.soil.aw_max[ly])) {
            dprintf("aw_max: %f\n", patch.soil.aw_max[ly]);
            dprintf("   awc: %f\n", patch.soil.soiltype.awc[ly]);
            fail("SSR irrigation work here relies on awc==aw_max; this is violated.");
        }
...
//    double inund_wcont = 0.99;
    ...
    if (ppft.phen > 0.0 && patch.soil.get_soil_water_upper()<restrict_irr_wcont)    {
        ...
        double wcont_1_opt = 0.0; // SSR: Only needed if inundated and !just_inund_upper
        
        // SSR learning: How much water does the plant want to take up? Given as a fraction of the maximum it possibly _could_ take up given today's pipes.
        double wr_opt = min(1.0, patch.wdemand / ppft.phen / pft.emax);

        if (wateruptake == WR_ROOTDIST) {

            // SSR learning: Determine how much water the plant would try to take up from the upper layer if no irrigation were added. Calculated as the total amount of water the plant wants/is capable of taking up (wr_opt*pft.emax) minus the amount the plant would try to take up from the lower layer absent irrigation.
            // TODO: SSR: Check the above note
            // SSR: This doesn't account for how the ratio of upper:lower soil moisture would actually end up looking after water_uptake(). In my version, I changed it so it does.
            // SSR learning: Only used for two-layer soil.
            if (inundated && ifactuallyinundate) {
                wcont_0_opt = inund_wcont;
                wcont_1_opt = inund_wcont; // SSR: Only needed if inundated and !just_inund_upper
            } else {
            ...
            if (!iftwolayersoil) {
                ...
                for (int i = 0; i<nsoillayer_tmp; i++) {
                    if (inundated && ifactuallyinundate) {
                        wcont_opt[i] = inund_wcont;
                    } else {
        ...
        // SSR learning: Turn on irrigation if (any) upper layer doesn't have enough water, according to how much the plant wants from that layer.
        if (iftwolayersoil) {
            if (inundated && ifactuallyinundate && !just_inund_upper && (wcont_0_opt > patch.soil.get_soil_water_upper() || wcont_1_opt > patch.soil.get_soil_water_lower()))
                irrigate_soil = true;
        ...
        /* SSR learning: If irrigating, this is where we determine how much is needed to get the soil to the right moisture
            simulate_day()-->irrigation().
         Here, we're in
            simulate_day()-->canopy_exchange()-->aet_water_stress()-->irrigated_water_uptake()
         Irrigation water (mm) required for (each) upper layer gets added to ppft.water_deficit_d.
         */
        // No irrigation when there is snow on the ground, or too much ice (if the latter restriction is enabled)
        if (irrigate_soil && patch.soil.dsnowdepth <= 0.001 && !patch.soil.ice_in_top_layer(nsoillayer_tmp, restrict_irr_ice)) {

                
            //
            //
            // SSR learning: If irrigating, this is where we determine how much is needed to get the soil in (each) upper layer to the moisture that the plant needs (or that it can maximally take up, whichever is lower). This will be the amount of water that gets added to the soil later in this function. It's also the amount that, in irrigation(), will be saved as today's irrigation for diagnostic variables.
            //
            //
            ...
            if (iftwolayersoil) {
                ...
                // If adding that much water would put us over the limit once we consider ice content, reduce water added.
                if (consider_ice && wcont_0 + ice_0 + water_to_add_ly[0]/awc0 > 1.0) {
                    // Apparently, wcont_cp[i] + ice_cp[i] can be slightly but non-negligibly >1.0
                ...
                if (inundated && ifactuallyinundate && !just_inund_upper) {
                    water_to_add_ly[1] = max(0.0, (wcont_1_opt - wcont_1) * awc1);
                    // If adding that much water would put us over the limit once we consider ice content, reduce water added.
                    if (consider_ice && wcont_1 + ice_1 + water_to_add_ly[1]/awc1 > 1.0) {
                        // Apparently, wcont_cp[i] + ice_cp[i] can be slightly but non-negligibly >1.0
                ...
                for (int i = 0; i<nsoillayer_tmp; i++) {
                    if (add_water[i]) {
                        ...
                        if (consider_ice && wcont_cp[i] + ice_cp[i] + water_to_add_ly[i]/patch.soil.soiltype.awc[i] > 1.0) {
                            // Apparently, wcont_cp[i] + ice_cp[i] can be slightly but non-negligibly >1.0
            ...
            // TODO: SSR: This uses the conventions normally associated with !iftwolayersoil, but presumably applies to 2-layer soil as well. Does that imply that the rest of the code in this function (and elsewhere?) can be similarly generalized for simplicity?
            for (int ly = 0; ly < nsoillayer_tmp; ly++) {
                if (add_water[ly]) {
                    // TODO: SSR: What does the below comment mean?
                    // Use: wcont[ly - IDX] = Faw_layer[ly - IDX] / soiltype.awc[ly - IDX];
                    Fw_liq_layer[ly] = soil.get_layer_soil_water(ly) * soil.soiltype.awc[ly]; // mm
                    Fw_ice_layer[ly] = soil.get_layer_soil_ice_mm(ly); // mm

                    // Water in this layer
                    
                    if (consider_ice) {
                        potential_layer[ly] = soil.aw_max[ly] - Fw_liq_layer[ly] - Fw_ice_layer[ly];
                    } else {
                        potential_layer[ly] = soil.aw_max[ly] - Fw_liq_layer[ly];
                    }
            ...
            //
            //
            // SSR learning: Actually add the irrigation to the soil.
            //
            //
            ...
            double new_wcont;
            for (int s=0; s<nsoillayer_tmp; s++) {
                ...
                if (add_water[s]) {
                    ...
                    new_wcont = Fw_liq_layer[s] / soil.soiltype.awc[s];
                ...
                // Error check
                if (!iftwolayersoil) {
                    if (fabs(wcont_cp[s] + too_much_ice_ly[s] - wcont_opt[s]) > 0.0001) {
                        dprintf("   wcont_opt[%d]: %f\n", s, wcont_opt[s]);
                        dprintf("    wcont_cp[%d]: %f\n", s, wcont_cp[s]);
                        dprintf("      ice_cp[%d]: %f\n", s, ice_cp[s]);
                        if (consider_ice) {
                            dprintf("too_much_ice[%d]: %f\n", s, too_much_ice_ly[s]);
                        }
                        fail("irrigated_water_uptake - error in the water balance!\n"); // SSR: Was originally just dprintf()
                        return -9999.0;
                    }
                }
            ...
            if (iftwolayersoil) {
                ...
                // TODO: SSR: Need to add check for lower layer as well (in case of inundation with full soil considered)
                if (fabs(new_wcont + too_much_ice_ly[0] - wcont_0_opt) > 0.0001) {
                    fail("irrigated_water_uptake - error in the water balance!\n"); // SSR: Was originally just dprintf()
                    return -9999.0;
                }
            }
    ...
    //
    //
    // SSR learning: Actually calculate the value that's really needed from this function: wr (plant water uptake as fraction of maximum possible)
    //
    //
...
void aet_water_stress(Patch& patch, Vegetation& vegetation, const Day& day) {
    ...
    for (int p=0; p<npft; p++) {
        ...
        if (day.isstart || ((spft.irrigated || spft.inundated) && pft.id == patch.stand.pftid)) {
            ...
            if ((spft.irrigated || spft.inundated) && pft.id == patch.stand.pftid) {
                wr = irrigated_water_uptake(patch, pft, day, spft.inundated);
            }
            else {
    ...
    if (date.day == 0) {
        ...
        while (vegetation.isobj) {
            ...
            if (!negligible(indiv.cmass_leaf) && !negligible(indiv.nmass_leaf))
             ...
             //TODO indiv.PODy = 0.0;
             indiv.PODy_yearly = 0.0;
             indiv.fraction_dead_leaf_yearly = 0.0;
...
#define TESTYEAR 1999
#define SUBDAILYGC

void ozone_stress(Patch& patch, Vegetation& vegetation) {
    if (date.year > nyear_spinup) {
        // diffusion scaling constant of ozone K_O3
        ...
        // O3 deposition from Cambell & Norman, 1998 and Franz, 2017
        const double Zm = 0.1 * height_veg; // roughness parameter
    ...
    #undef TESTYEAR
            ...
            o3a = 22.0*(1.0 + 2.0/3.0*sin(PI*2.0*(double)date.day/365.0-PI/2.0))*1e-9;
    #ifdef TESTYEAR

                //o3a = 1.0e-9; //  ppbv

                if(date.get_calendar_year() > TESTYEAR) {
    ...
    */;
    ...
    //double o3c = o3a * (1.0 - Ra / (Ra + Rb + Rc)); //nmol/m3
    double o3c = o3a * mol_per_m3 * (1.0 - Ra / (Ra + Rb + Rc)); //nmol/m3

    vegetation.firstobj();
    double gc_o3_integrator = 0.33; // Heath et al 2009, integrator considers the mismatch among peaks for ozone, conductance and ascorbate
    while (vegetation.isobj) {
        ...
        /*if (cp.dev_stage < 0.5) {
            indiv.POD6 = 0.0; // basically insensitive to O3 in the beginning of the growing season
        }*/

        //gc = indiv.photosynthesis.gcbase * 1e-3;
...
#undef SUBDAILYGC
#ifdef SUBDAILYGC
        double Fo3 = 0.0;
        int hours = (int)floor(cl.daylength);

        for (int i = 0; i < hours; i++) {
            double pi = PI;
            double gc_t = gc * (1.0 + cos((double)i * 2.0 * pi / ((double)hours) + pi));
            Fo3 += max(((gc_t * o3c * mol_per_m3) - 6.0e-9), 0.0) / cl.daylength;
        }
#else
            //double Fo3 = ((gc_c / K_o3 * o3c) - ppft.pft.PODy);//O3 flux over treshold nmol/m2*s
        double Fo3 = (((gc_c / K_o3) * o3c) - ppft.pft.PODy);//O3 flux over treshold nmol/m2*s
#endif

        double dPODy = Fo3 / 1e6 * daylength * gc_o3_integrator;//convert to milli mole/m2
        ...
        if (dPODy > 0.0) {
            ...
            if (indiv.cmass_leaf_today() > 0.0) {
                double leafn = indiv.nmass_leaf;
                double leafc = indiv.cmass_leaf_today();
                double dleaf = (indiv.pft.PODy_m + indiv.pft.PODy_k * indiv.PODy) * dPODy; // Senescence effect on the leaf
                //Only if the O3 effect is negative
                if (dleaf < 0.0) {
                    if (indiv.istruecrop_or_intercropgrass()) {
                        ...
                        if (cp.dev_stage < 0.95) {
                            ...
                            ci.grs_cmass_dead_leaf += leafc * fabs(dleaf);
                            ci.grs_cmass_leaf += leafc * dleaf;
                            indiv.nmass_leaf += leafn * dleaf;
                            ci.nmass_dead_leaf += leafn * fabs(dleaf);
                        }
                    }
                    else {
                        if (indiv.phen < 0.95) { //This is just a placeholder for more detailed description on the seasonality of the ozone sensitivities of different pfts.
                            ...
                            indiv.cmass_dead_leaf += leafc * fabs(dleaf);
                            indiv.nmass_dead_leaf += leafn * fabs(dleaf);
                            //TODO implement a function for active nmass_leaf
                            if (indiv.cmass_dead_leaf > 0.9 * indiv.cmass_leaf) {
                                indiv.cmass_dead_leaf = 0.9 * indiv.cmass_leaf;
                            }
                            if (indiv.nmass_dead_leaf > 0.9 * indiv.nmass_leaf) {
                                indiv.nmass_dead_leaf = 0.9 * indiv.nmass_leaf;
                            }
                        }
                    }
                }
            ...
            indiv.PODy += dPODy;
            indiv.PODy_yearly += dPODy;
            indiv.fraction_dead_leaf_yearly = indiv.cmass_dead_leaf;
...
double bnf_func_developmentstage (double ds, Pft& pft) {
    double gmin = pft.bnf_ds_min;
    double goptL = pft.bnf_ds_opt_low;
    double goptH = pft.bnf_ds_opt_high;
    double gmax = pft.bnf_ds_max;
    double f = 0.0;
    double g = ds;
    if (pft.phenology==CROPGREEN) {
        g/=2.0;
...
double bnf_func_wcont(double w, Pft& pft) {
    double Wa = pft.bnf_wcont_min;
    double Wb = pft.bnf_wcont_max;
    double phi1 = pft.bnf_wcont_intercept;
    double phi2 = pft.bnf_wcont_rate;
    double f = 0.0;
    if (w <= Wa) {
        f = 0.0;
    } else {
...
double bnf_func_temperature (double T, Pft& pft) {
    double Tmin = pft.bnf_t_min;
    double ToptL = pft.bnf_t_opt_low;
    double ToptH = pft.bnf_t_opt_high;
    double Tmax = pft.bnf_t_max;
    double f = 0.0;
    if (T < Tmin || T > Tmax) {
        f = 0.0;
    } else {
...
void bnf(Patch& patch, Vegetation& vegetation) {
    if (!(vegetation.patch.stand.landcover == CROPLAND && ifnlim)) {
    //if (!(vegetation.patch.stand.is_true_crop_stand() && ifnlim)) {
        return;
    }
    ...
    while (vegetation.isobj) {
        Individual& indiv = vegetation.getobj();
        if ((char*)indiv.pft.name=="CC3G_ic"){
            int n = 1;
        }
        if (indiv.pft.fixer) {
            ...
            if (ndeficit > 0.0 && indiv.dnpp > 0.0) {
                ...
                double f_T, f_w, f_ds;
                // The maximum fraction of the daily NPP that is available for BNF.
                double f_npp = 0.9;

                int crop;
                if (indiv.pft.phenology==CROPGREEN) {
                    f_ds = bnf_func_developmentstage(ppftcrop.dev_stage, indiv.pft);
#ifdef MAXBNFNPP
                    // Eq. 3 & 5, Olin 2015
                    //
                    ...
                    f_ds = bnf_func_developmentstage(indiv.phen, indiv.pft); // phen E[0,1]
                }
                f_T = bnf_func_temperature(patch.soil.get_soil_temp_25(), indiv.pft);
                f_w = bnf_func_wcont(patch.soil.get_soil_water_upper(), indiv.pft);

                double potnfix = f_T * f_w * f_ds * indiv.cmass_root_today() * indiv.pft.bnf_max * 2.0;
                ...
                ppftcrop.f2_mod = nfix_cost / indiv.dnpp;
                ...
                //indiv.nmass_sap       += indiv.sapfndemand   * nfix_day;
...
//                if (nfix_day > 0.0) {
//                    dprintf("N def: %g \t fix: %g ratio: %g PFT: %s\n", ndeficit, nfix_day, nfix_day/ndeficit,(char*)indiv.pft.name);
//                }
            }
        }
        vegetation.nextobj();
...
// Heath R, Lefohn A, Musselman R. 2009.  Temporal processes that contribute to nonlinearity
// in vegetation responses to ozone exposure and dose. Atmospheric Environment 43: 18
