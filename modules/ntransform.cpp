...
void ntransform(Patch& patch, Climate& climate) {
	if (ifntransform) {
		...
		//if(date.get_calendar_year()<=1901 && (date.day < 2 || date.day > 360)){
		//	dprintf("%d %d %g %g %g %g %g %g %g %g %g %g\n",date.get_calendar_year(),date.day,soil.get_soil_water_upper(),soil.NH4_mass,soil.NO3_mass,soil.NO2_mass,soil.labile_carbon,soil.N2O_mass,soil.NO_mass,soil.N2_mass,fluxes.get_annual_flux(Fluxes::NO_SOIL),fluxes.get_annual_flux(Fluxes::N2O_SOIL));
		//}
		// NH3 volatilization
