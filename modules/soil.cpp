...
void Soil::hydrology_lpjf_twolayer(const Climate& climate, double fevap) {
	...
	double perc = 0.0;
	bool disable_perc_thisstand = patch.stand.is_true_crop_stand() && stlist[patch.stand.stid].get_management(patch.stand.current_rot).disable_perc;
	if (percolate && !disable_perc_thisstand) {
...
bool Soil::ice_in_top_layer(int nlayer, double max_ice_fraction) {
	...
	//	max_ice_fraction is optional; default 0.05, which avoids irrigation restrictions when there are tiny amounts of ice in the soil
...
void Soil::hydrology_lpjf(const Climate& climate, double fevap) {
	...
	for (int ly = 0; ly < NSOILLAYER; ly++) {
		...
		if (potential_layer[ly] < -1.0 * maxerr) {
			...
			dprintf("ERROR: Soil::hydrology_lpjf - error in a soil layer's water balance! potential_layer#: %d\n", ly);
            dprintf("      aw_max: %f\n", aw_max[ly]);
            dprintf("         awc: %f\n", soiltype.awc[ly]);
            dprintf("wcont (frac): %f\n", wcont[ly]);
            dprintf("  wcont (mm): %f\n", Faw_layer[ly]);
            dprintf("         ice: %f\n", ice_layer[ly]);
            fail("This is caused by ice! Why isn't it taken care of by ice-content limit on irrigation?\n");
		}
		else {
	...
	if (percolate && !disable_perc_thisstand) {
		...
		if (Faw_perc_layer_2 > MIN_WATER_PERC) {
...
// SSR: return ice fraction for a certain layer, in same units as wcont.
// Note that the denominator of Frac_ice is layer thickness, which is why we have to do the conversion.
...
// SSR: return ice (mm) for a certain layer
...
// SSR: return ice fraction for a subset of soil layers (after Soil::get_soil_water())
// Note that the denominator of Frac_ice is layer thickness, which is why we have to do the conversion.
double Soil::get_soil_ice(int layer1, int layer2) const {

    double total_ice = 0.0; // ice for the thick soil layer (mm)
    double total_capacity = 0.0;
    ...
    for (int ly = layer1; ly < layer2; ly++) {
        ...
        total_ice += Frac_ice[ly + IDX] * Dz[ly + IDX]; // mm
        total_capacity += aw_max[ly]; // mm
    }
...
// SSR: return Frac_ice for the upper 50cm (upper true) or lower (100cm) soil layer (upper false)
double Soil::get_soil_ice_upper() const {
    return get_soil_ice(0, nsublayer1);
}

// SSR: return Frac_ice for the lower (100cm) soil layer
