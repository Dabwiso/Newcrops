...
/// $Date: 2020-04-06 20:16:26 +0100 (Mon, 06 Apr 2020) $
///
///////////////////////////////////////////////////////////////////////////////////////
...
void decayrates_century(Soil& soil, double temp_soil, double wcont_soil, bool tillage) {
	...
	bool ismineralwetland = soil.patch.stand.is_true_wetland_stand() || soil.patch.stand.isinundated;
...
void soilnadd(Patch& patch) {
	...
	if (nmin_avail < NMASS_SAT || patch.isinundated) {
		double k = 1.0;

		if (patch.isinundated) {
			k = 7.0/3.0;
		}
		const double daily_nfix = soil.anfix_calc / (double)date.year_length() * k;

		if (nmin_avail + daily_nfix < NMASS_SAT || patch.isinundated) {
