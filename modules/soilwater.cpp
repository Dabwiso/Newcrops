		...
		if (patch.stand.is_true_wetland_stand()) {
			...
			if (total_potential > 0.0) {
				...
				for (int ly = 0; ly<NSOILLAYER; ly++) {
					...
					double water_input_ly = min(potential_layer[ly], soil.rain_melt);

					// Add water to the layer, and update wcont and Frac_water for this layer:
					soil.add_layer_soil_water(ly, water_input_ly);
					
					// Record the water added to this wetland today
					if (ifsaturatewetlands || patch.isinundated)
						patch.wetland_water_added_today += water_input_ly;
					
					// Reduce rain_melt by water added
					soil.rain_melt -= water_input_ly;
					if (soil.rain_melt == 0.0) {
						break;
					}
 ...
 */
// SSR learning: This function only serves to update the diagnostic variables patch.irrigation_{d,y}. It does NOT affect how much water actually gets added to the soil.
void irrigation(Patch& patch) {
	...
	if (!(patch.stand.isirrigated || patch.isinundated || patch.stand.isinundated)) {
		return;
	}
	...
	for (int i = 0; i < npft; i++) {
		...
		if ((patch.stand.pft[i].irrigated || (ifactuallyinundate && patch.stand.pft[i].inundated)) && ppft.growingseason()) {
			if (ppft.water_deficit_d < 0.0) {
				fail("irrigation: Negative water deficit for PFT %s!\n", (char*)ppft.pft.name);
			}
			patch.irrigation_d += ppft.water_deficit_d;
		} else if (!ifactuallyinundate && patch.stand.pft[i].inundated && ppft.growingseason()) {
            
            // SSR learning: For inundated crops, we throw out the water_deficit_d that was calculated in irrigated_water_uptake(). Instead, we just say, "how much water is required to saturate the upper layer?"
            // SSR idea: Add some more mm on top of that to reflect typical flooding depth.
            double water_deficit_d = 0.0;
            for (int l = 0; l < 2;l++) {
                double soilwater = 0.0;
                if(l){
                    soilwater = soil.get_soil_water_upper();
                } else {
                    soilwater = soil.get_soil_water_lower();
                }
                water_deficit_d += (1.0 - min(1.0, soilwater))*patch.soil.soiltype.gawc[l];
            }
