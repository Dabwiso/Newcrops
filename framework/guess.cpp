...
void Stand::init_stand_lu(StandType& st, double fraction) {
	...
	if (mt0.hydrology == INUNDATED) {
		isinundated = true;								// First main crop, may change during crop rotation
		if (pftid >= 0)
			pft[pftid].inundated = true;
	}
...
void Stand::rotate() {
	...
	if (pftid >= 0 && stid >= 0) {
		...
		else if (mt.hydrology == INUNDATED) {
			isinundated = true;
			standpft.inundated = true;
		}
		else {
			isirrigated = false;
			standpft.irrigated = false;
			isinundated = false;
			standpft.inundated = false;
		}
