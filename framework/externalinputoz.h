...
/// Class that deals with the ozone input from text files
class OzoneInput {
...
public:
	...
	/// Constructor
	OzoneInput();
	/// Opens ozone data files
	void init();
	/// Loads fertilisation, sowing and harvest dates from input files
	bool loadozone(double lon, double lat);
	/// Gets management data for a year
	void getozone(Gridcell& gridcell);

private:

	/// Input objects for each management text input file
	InData::TimeDataD ozone;
	
	/// Files names for management input file
	xtring file_ozone;

	/// Gets sowing date data for a year
	void getozonedates(Gridcell& gridcell);
