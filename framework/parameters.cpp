...
void plib_declarations(int id,xtring setname) {
	...
	case BLOCK_GLOBAL:
        ...
        declareitem("ifactuallyinundate",&ifactuallyinundate,1,CB_NONE,"Whether to actually add all water to inundated crops (true, new behavior) or just add enough for plant demand and then add the rest in diagnostic outputs only (false, old behavior).");
			// declareitem("k_N",    &k_N,    0.0001, 1.0, 1,CB_NONE, "Constant in denitrification");
        declareitem("restrict_irr_wcont",&restrict_irr_wcont,0.0,2.0,1,CB_NONE,"Threshold for upper soil moisture (wcont) above which water addition (for irrigation and inundation) are disabled. Default 2.0 (just to ensure that this restriction never happens regardless of floating-point errors); previous value 0.9.");
		declareitem("restrict_irr_ice",&restrict_irr_ice,0.0,2.0,1,CB_NONE,"Threshold for upper soil ice fraction above which water addition (for irrigation and inundation) are disabled. Default 0.05; maximum 2.0 (just to ensure that this restriction never happens regardless of floating-point errors). Note that this is tripped if ANY upper soil layer has ice above the threshold.");
	...
	case BLOCK_PFT:
		...
		//NEW CROPS
		declareitem("a_fphu_ds_1",&ppft->a_fphu_ds_1,-1000.0,1000.0,1,CB_NONE,
					"constant in the linear interpolation between f_phu and ds.");
		declareitem("a_fphu_ds_2",&ppft->a_fphu_ds_2,-1000.0,1000.0,1,CB_NONE,
					"constant in the linear interpolation between f_phu and ds.");
		declareitem("b_fphu_ds_1",&ppft->b_fphu_ds_1,-1000.0,1000.0,1,CB_NONE,
					"coefficient in the linear interpolation between f_phu and ds.");
		declareitem("b_fphu_ds_2",&ppft->b_fphu_ds_2,-1000.0,1000.0,1,CB_NONE,
					"coefficient in the linear interpolation between f_phu and ds.");
		declareitem("fphu_anthesis",&ppft->fphu_anthesis,0.0,1000.0,1,CB_NONE,
					"f_phu at which to switch between the first and the linear interpolation between f_phu and ds.");

		// gmin  goptL goptH gmax Wb   Wa   phi1   phi2  Tmin ToptL ToptH Tmax MaxBNF
		declareitem("bnf_ds_min",&ppft->bnf_ds_min,-1.0,2.0,1,CB_NONE,
			"Development stage when BNF can start.");
		declareitem("bnf_ds_max",&ppft->bnf_ds_max,-1.0,2.0,1,CB_NONE,
			"Development stage after BNF will not happen.");
		declareitem("bnf_ds_opt_low",&ppft->bnf_ds_opt_low,-1.0,2.0,1,CB_NONE,
			"Development stage when BNF is at its optimum.");
		declareitem("bnf_ds_opt_high",&ppft->bnf_ds_opt_high,-1.0,2.0,1,CB_NONE,
			"Development stage when BNF is at its optimum.");

		declareitem("bnf_t_min",&ppft->bnf_t_min,-1.0,200.0,1,CB_NONE,
			"Temperature when BNF can start.");
		declareitem("bnf_t_max",&ppft->bnf_t_max,-1.0,200.0,1,CB_NONE,
			"Temperature stage after BNF will not happen.");
		declareitem("bnf_t_opt_low",&ppft->bnf_t_opt_low,-1.0,200.0,1,CB_NONE,
			"Temperature stage when BNF is at its optimum.");
		declareitem("bnf_t_opt_high",&ppft->bnf_t_opt_high,-1.0,200.0,1,CB_NONE,
			"Temperature stage when BNF is at its optimum.");

		declareitem("bnf_wcont_min",&ppft->bnf_wcont_min,-1.0,2.0,1,CB_NONE,
			"Water content when BNF can start.");
		declareitem("bnf_wcont_max",&ppft->bnf_wcont_max,-1.0,2.0,1,CB_NONE,
			"Water content after which the effect on BNF will be 1.");
		declareitem("bnf_wcont_intercept",&ppft->bnf_wcont_intercept,-2.0,2.0,1,CB_NONE,
			"Constant in the Water content effect function.");
		declareitem("bnf_wcont_rate",&ppft->bnf_wcont_rate,-1.0,200.0,1,CB_NONE,
			"Constant in the Water content effect function.");

		declareitem("bnf_max",&ppft->bnf_max,-1.0,2.0,1,CB_NONE,
					"Maximum BNF for this PFT, unit kgN/m-1d-1.");
...
//		declareitem("irrigation",&pmt->firr,0.0,1.0,1,CB_NONE,"Irrigation of crop");
		...
		declareitem("disable_perc",&pmt->disable_perc,1,CB_NONE,"Whether to disable percolation");
...
//				declareitem("irrigation",&pst->management.firr,0.0,1.0,1,CB_NONE,"Irrigation of crop 1");
				...
				declareitem("disable_perc",&pst->management.disable_perc,1,CB_NONE,"Whether to disable percolation in management 1");
			}
			else if(i == 1) {
...
void plib_callback(int callback) {
	...
	case CB_MTHYDROLOGY:
		...
		else if (strparam.upper()=="INUNDATED") pmt->hydrology = INUNDATED;
		else 
		{
			sendmessage("Error",
	...
	case CB_CHECKGLOBAL:
		...
		if (run_landcover) {
			...
			if(run[CROPLAND]) {
                ...
                // SSR
                if (!itemparsed("ifactuallyinundate")) badins("ifactuallyinundate");
			}
		}
