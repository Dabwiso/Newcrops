...
public:
	...
	double nyears;
	/// hydrology (RAINFED,IRRIGATED,INUNDATED)
...
//	double firr;
	...
	bool multicrop;
	/// Whether to disable percolation
	bool disable_perc;
...
public:
	...
	// Parameters for the linear interpolation between f_phu and ds.
	double a_fphu_ds_1, a_fphu_ds_2, b_fphu_ds_1, b_fphu_ds_2, fphu_anthesis;
	...
	//Parameters describing the dependence on available soil water for plant BNF
	double bnf_wcont_min, bnf_wcont_max, bnf_wcont_intercept, bnf_wcont_rate;

	//Parameters describing the temperature dependence for plant BNF
	double bnf_t_min, bnf_t_max, bnf_t_opt_low, bnf_t_opt_high;
...
public:
	...
	Pft() {
		...
		// parameters for wheat, default if not supplied in the ins-file.
		a_fphu_ds_1 = -0.07;
		a_fphu_ds_2 = 0.2247;
		b_fphu_ds_1 = 2.45;
		b_fphu_ds_2 = 1.7753;
...
public:
    ...
    /// SSR: return Frac_ice for a certain layer
    ...
    /// SSR: As get_soil_water(), but for ice
    double get_soil_ice(int layer1, int layer2) const;
    
    /// SSR: As get_soil_water_upper(), but for ice
    double get_soil_ice_upper() const;

    /// SSR: As get_soil_water_lower(), but for ice
	...
	bool ice_in_top_layer(int nlayer, double max_ice_fraction = 0.05);
